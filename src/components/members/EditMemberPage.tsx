...
export const EditMemberPage: React.FC = () => {
  const { pageParams, getMember, updateMember, navigateTo } = useApp();
  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        contactNumber: (member.contactNumber || '').replace(/\D/g, '').slice(0, 11),
        propertyType: member.memberType === 'COMMERCIAL' ? 'Commercial' : 'Residential',
        address: member.address || member.houseNumber || '',
        monthlyAmount: member.monthlyAmount || 0,
      });

      let initFloors: string[] = ['Ground'];
      if (member.floors && Array.isArray(member.floors) && member.floors.length > 0) {
        initFloors = member.floors;
      } else if (member.plotNumber) {
        initFloors = member.plotNumber.split(',').map((s) => s.trim()).filter(Boolean);
      }
      setSelectedFloors(initFloors.length > 0 ? initFloors : ['Ground']);
    }
  }, [member]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateMember(member.memberId, {
        ...
        memberType: formData.propertyType.toUpperCase(),
        floors: finalFloors,
