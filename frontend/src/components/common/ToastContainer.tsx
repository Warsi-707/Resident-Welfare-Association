    ...
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2.5 max-w-sm w-full pointer-events-none">
      {toasts.map((toast) => {
        ...
        const borders = {
          ...
          info: 'border-purple-200 bg-white',
        };

        return (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-start gap-3 p-3.5 rounded-xl border shadow-lg ${borders[toast.type]} transition-all duration-200 animate-in slide-in-from-bottom-3`}
          >
            {icons[toast.type]}
